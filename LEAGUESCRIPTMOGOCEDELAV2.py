    ...
    if contours:
        ...
        if center_x is not None:
            ...
            if dist_sq < radius**2: #CE JE V RANGE MAX RANGE
                ...
                #L9 KITING DEMON OD CHAMPA
                dx = center_x - cx # DX POMENI DX=KOLKO JE LEVO/DESNO OD CHAMPA DY PA GOR DOL 
                dy = center_y - cy #ČE JE DX POZITIVEN JE SPODAJ OD ENEMY CHAMPA ČE JE DY POZITIVEN JE POD ENEMY CHAMPOM
                dist = np.sqrt(dx*dx + dy*dy)
                if dist > 0:
                    nx = dx / dist
                    ny = dy / dist

                    step = 20 # KOLKO PIXELS KITE STRAN
                    move_x = center_x + nx * step
                    move_y = center_y + ny * step
